-- script to create main patient-location activity fact for dental stats publication
-- first used for 2024/25 publication, code based on DS_CONT_ACTIVITY_FACT_2425
-- * patient location mapped from patient postcode up to ward, icb, region, national
-- * remove filter of active remissions as tax credit exemption data has changed after scheme end
-- * columns added to get dental care professional (DCP) flag and DCP type

--NSPL columns required for join: PCD, CTRY, LAUA, WARD, NHSER, ICB, LSOA11, ICB23CDH, ICB23NM, IMD_DECILE, IMD_RANK   
--laua, ward, sicbl, icb and region are structure at time of nspl aug 24 creation
--lsoa is 2011 census lsoa

--drop table ds_pat_activity_fact_2425 purge;
create table  ds_pat_activity_fact_2425 compress for  query high  as
...
--update ICB column value to account for Belfast patient postcode being mapped unexpectedly

UPDATE DS_PAT_ACTIVITY_FACT_2425
SET
    PAT_ICB = CASE POSTCODE
        WHEN 'BT9 6SU' THEN NULL
    END,
    PAT_ICB_CDH = CASE POSTCODE
        WHEN 'BT9 6SU' THEN NULL
    END,
    PAT_ICB_NM = CASE POSTCODE
        WHEN 'BT9 6SU' THEN NULL
    END
WHERE POSTCODE = 'BT9 6SU';

--select * from DS_PAT_ACTIVITY_FACT_2425
--where postcode = 'BT9 6SU'
