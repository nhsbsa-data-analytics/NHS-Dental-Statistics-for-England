...
--drop table ds_workforce_fact_2425 cascade constraints purge;

create table ds_workforce_fact_202503_wales compress for query high as
with
...
--map 2019/20 to 2024/25 data to latest contract details as of 202503
,cont as (
  select
    year_month
    ,contract_number
...
,fact as  (
  select
    ...
    ,cont.commissioner_code
    ,cont.ltst_comm_code
    ,cont.ltst_comm_name
    ,cont.ons_country
    ,cont.provider_number
    ,cont.contract_number
    ,cont.contract_type
    ,cont.paid_by_bsa
    ,cont.ppc_address_postcode
    ,ons.ccg_code
    ,sum(fact.uda)  as  uda
    ,sum(fact.uoa)  as  uoa    
  from
...
--    and fact.year_month     = tdim.year_month
  ...
  left outer join
    ...
    on  cont.ppc_address_postcode = ons.postcode 
  where 1 = 1
    and fact.ons_country  = 'W92000004'
    and fact.treatment_year between '2019/2020' and '2024/2025'
  group by
    fact.treatment_year
    ,perf.performer_number
    ,perf.title
