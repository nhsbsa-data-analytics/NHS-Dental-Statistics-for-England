#Script to format data extracts for use in charts and tables in narrative markdown
#custom functions from nhsbsaVis package used to create chart objects using highcharter

#data objects (eg. table1ai) will be available in R envrironment if already called in pipeline
#from:
# - import_contract_activity_national_tables.R (data objects used in figure 1 to 6)
# - import_contract_activity_geo_tables.R (data objects used in figure 7 and 8)
# - import_patient_activity_geo_tables (data objects used in figure 7 and 8)
# - england_workforce_pipeline_2425.R for workforce data (data objects used in figure 9 and 10)

#TO DO: add custom rounding function to functions folder
#this function is only used in the chart creation call so doesn't need to be migrated
# round_any <-
...
# chart input data, add group totals
fig_3_data <- table1c |>
  dplyr::select(`Financial year`,
                `Patient_Type`,
                `Total`) |>
...
# # fix issue with label spacing in final visualisation
...
# formatted table data for user download
# UDA totals supplied directly to force levels/rounding and display correctly
# use fig_4_data instead if formatting issues can be resolved
...
# additional argument to try and fix issue with decimals still appearing
table_4_data_format <- fig_4_data |>
  dplyr::mutate(`Total UDA` = substr(`Total UDA`, 1, nchar(`Total UDA`))) |>
  dplyr::mutate(`Total UDA` = format(`Total UDA`, big.mark = ","))

# previous code below shows data to 1 decimal place 
# but argument to apply thousand number separators does not work as expected
# table_4_data <- fig_4_data |>
#   dplyr::mutate(`Total UDA` = sprintf(`Total UDA`, fmt = '%#.1f')) |>
#   dplyr::mutate(`Total UDA` = format(`Total UDA`, big.mark = ","))

#Figure 5 patient revenue ------------------------------------------------------
...
# try supplying values directly as vectors 
...
#try recreating chart input data from vectors above
...
#Figure 6 Dental COTs by DCP status --------------------------------------------
...
# # code to create column chart 
# # unused in 2024/25 narrative as map was used instead
...
# currently calls icb lookup again 
# if ICB 2023 names and codes already in data then this step not needed
...
# get shapefiles of ICB boundaries to supply to map chart function
# icb_geo_data <-
#   icb_geo_data(
...
# supply mean adult percentage as context above map
...
# supply mean national dentists to display above map
nat_avg_dentists <- "42"
