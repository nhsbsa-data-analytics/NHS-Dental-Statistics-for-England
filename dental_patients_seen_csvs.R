...
unique_years <- unique(dental_patients_seen_data_overall$FINANCIAL_YEAR)

#loop through financial years to filter data and save a .csv
for(i in unique_years) {
  print(i)
year_data <- dental_patients_seen_data_overall |>
...
fwrite(year_data, 
       paste0("Y:\\Official Stats\\Dental\\2024_25\\csvs\\CSV outputs\\Patients Seen csvs\\dental_patients_seen_",
              substr(i, 1, 4),
              "_",
              substr(i, 8, 9),
              ".csv"))
...
setwd("Y:\\Official Stats\\Dental\\2024_25\\csvs\\CSV outputs\\Patients Seen csvs")
...
check <- dental_patients_seen_data_overall |>
  filter(FINANCIAL_YEAR == "2024/2025",
         PSEEN_END_DATE == as.Date("2025-03-31"),
         PATIENT_TYPE == "Adult")
